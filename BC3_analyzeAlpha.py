...
file_path = "dataset_bc3.xlsx"  
df = pd.read_excel(file_path, sheet_name="Sheet1", usecols=["Angle Alpha", "Amount earth has deviated because of Rogue"])

# Group by velocity and sum deviation amounts
deviation_summary = df.groupby("Angle Alpha")["Amount earth has deviated because of Rogue"].sum().reset_index()

# Display summarized results
print(deviation_summary.head())  # Print only first few rows

# Create the figure and axis
fig, ax = plt.subplots(figsize=(10, 7))

# Get x-axis and y-axis ranges
x_min = deviation_summary["Angle Alpha"].min()
x_max = deviation_summary["Angle Alpha"].max()
y_min = 0
...
# Customize labels
ax.set_xlabel("Angle Alpha (degrees)", fontsize = 16)
...
ax.set_xticks(np.arange(x_min, x_max + 1, 45))
plt.xticks(rotation=20, fontsize=12)

# Save and show the plot
file_name = "bc3_Analysis_Alpha.png"
