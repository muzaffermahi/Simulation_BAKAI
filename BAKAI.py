...
# Total iterations
total_D = 3  # D ranges from 1 to 3 (inclusive)
total_S = 9  # S ranges from 1 to 9 (inclusive)
total_V = 19  # V ranges from 1 to 19 (inclusive)
total_iterations = total_D * total_S * total_V

# Split the iterations among processes
iterations_per_process = total_iterations // size
remaining_iterations = total_iterations % size

# Each process calculates its portion of iterations
start = rank * iterations_per_process + min(rank, remaining_iterations)
end = start + iterations_per_process + (1 if rank < remaining_iterations else 0)

c = int(0)
for i in range(start, end):
    D = i // (total_S * total_V) + 1
    S = (i % (total_S * total_V)) // total_V + 1
    V = (i % total_V) + 1
    ...
    def SmallAx():
        ...
        output_dir = "DATASET_A_photos_SmallAx"
    ...
    def BigAx():
        ...
        ax.grid(True)
                # If there is no file create one
        output_dir = "DATASET_A_photos_BigAx"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
