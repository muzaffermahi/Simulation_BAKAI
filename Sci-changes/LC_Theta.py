# Muzaffer Mahi Can, 03/02/2025(I'm getting old), beginning date of the simulation is 16/05

#In LC, you basically plug in values from your BC's results, which contains alpha, sub, Velocities etc
...
dt = 0.1  # Time that acceleration is assumed to be constant, smaller is better

total_time = 86400 * 365.24219/1000  # Total simulation time in seconds
I_range = 20000
...
#Positions L
vctr_position_Rogue_L = np.zeros((I_range, 3))
vctr_position_earth_L = np.zeros((I_range, 3))
vctr_position_moon_L = np.zeros((I_range, 3))
vctr_position_earth0_L = np.zeros((I_range, 3))
vctr_position_mercury_L = np.zeros((I_range, 3))
vctr_position_venus_L = np.zeros((I_range, 3))
vctr_position_mars_L = np.zeros((I_range, 3))
vctr_position_jupiter_L = np.zeros((I_range, 3))
vctr_position_uranus_L = np.zeros((I_range, 3))
vctr_position_saturn_L = np.zeros((I_range, 3))
...
#Velocities L
vctr_velocity_Rogue_L = np.zeros((I_range, 3))
vctr_velocity_earth_L = np.zeros((I_range, 3))
vctr_velocity_moon_L = np.zeros((I_range, 3))
vctr_velocity_earth0_L = np.zeros((I_range, 3))
vctr_velocity_mercury_L = np.zeros((I_range, 3))
vctr_velocity_venus_L = np.zeros((I_range, 3))
vctr_velocity_mars_L = np.zeros((I_range, 3))
vctr_velocity_jupiter_L = np.zeros((I_range, 3))
vctr_velocity_saturn_L = np.zeros((I_range, 3))
vctr_velocity_uranus_L = np.zeros((I_range, 3))
...
vctr_velocity_Rogue_L[0] = np.array([0, 0, -180])


theta_rad0 = np.arctan2(-1.249081082441206E+08, -8.720402184155567E+07)
theta_deg0 = theta_rad0 % 360
print(theta_deg0)
theta_rad = np.arctan2(Y_Earth, X_Earth)
theta_deg = theta_rad % 360
...
def compute_position(total_time, dt, G, M_SUN, M_earth, M_VENUS, M_MARS, M_mercury, M_JUPITER, M_SATURN, M_NEPTUNE, M_URANUS, M_Rogue, M_Moon,
                                ...
                                vctr_velocity_jupiter_F,vctr_velocity_saturn_F,vctr_velocity_uranus_F,theta_rad,theta_deg): 
    ...
    for n in range(1, int(total_time/dt )):
        ...
        #if(((np.arctan2(vctr_position_earth[n][1],vctr_position_earth[n][0])) % 360) == 357):
         #    print("1 year have passed")
...
for i in range(0, I_range):
    ...
    vctr_position_mars[0] = vctr_position_mars_L[i]
    ...
    with open(csv_file_path, 'a', newline='') as file:
        ...
        writer.writerow([
        vctr_position_Rogue_L[i][0], vctr_position_Rogue_L[i][1], vctr_position_Rogue_L[i][2],  # Rogue position
        vctr_position_earth_L[i][0], vctr_position_earth_L[i][1], vctr_position_earth_L[i][2],  # Earth position
        vctr_position_moon_L[i][0], vctr_position_moon_L[i][1], vctr_position_moon_L[i][2],      # Moon position
        vctr_position_mercury_L[i][0], vctr_position_mercury_L[i][1], vctr_position_mercury_L[i][2],      # Moon position
        vctr_position_venus_L[i][0], vctr_position_venus_L[i][1], vctr_position_venus_L[i][2],      # Moon position
        vctr_position_mars_L[i][0], vctr_position_mars_L[i][1], vctr_position_mars_L[i][2],      # Moon position
        vctr_position_jupiter_L[i][0], vctr_position_jupiter_L[i][1], vctr_position_jupiter_L[i][2],      # Moon position
            
        ])

    with open(Degree_file_path, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([
       
            np.degrees(np.arctan2(vctr_position_earth_L[i][1],vctr_position_earth_L[i][0])),
            np.sqrt((vctr_position_earth_L[i][0]**2) + (vctr_position_earth_L[i][1]**2))

        ])
            
    
    gc.collect()
    vctr_position_earth[:,-2].fill(0)
    vctr_position_Rogue[:,-2].fill(0)
    vctr_position_moon[:,-2].fill(0) 
    vctr_position_mercury[:,-2].fill(0)
    vctr_position_venus[:,-2].fill(0)
    vctr_position_mars[:,-2].fill(0) 
    vctr_position_jupiter[:,-2].fill(0)

    vctr_velocity_earth[:,-2].fill(0)
    vctr_velocity_Rogue[:,-2].fill(0)
    vctr_velocity_moon[:,-2].fill(0) 
    vctr_velocity_mercury[:,-2].fill(0)
    vctr_velocity_venus[:,-2].fill(0)
    vctr_velocity_mars[:,-2].fill(0) 
    vctr_velocity_jupiter[:,-2].fill(0)

    vctr_acceleration_earth[:,-2].fill(0)
    vctr_acceleration_Rogue[:,-2].fill(0)
    vctr_acceleration_moon[:,-2].fill(0)
    vctr_acceleration_mercury[:,-2].fill(0)
    vctr_acceleration_venus[:,-2].fill(0)
    vctr_acceleration_mars[:,-2].fill(0) 
    vctr_acceleration_jupiter[:,-2].fill(0)

    ending_time = time.perf_counter()
    passed_time = ending_time - beginning_time
    print(f"Remaining Time: {passed_time*(I_range - i)/60}min")
...
def Plot3D():
    ...
    photo_file_path = "Dataset_LC3_photos"
    if not os.path.exists(photo_file_path):
        os.makedirs(photo_file_path)

    file_name = "Dataset_LC_30.05.png"
...
def AxVid():
        ...
        line_mercury, = ax.plot([], [], [], 'b', label='Mercury')
        line_venus, = ax.plot([], [], [], 'y', label='Venus')
        ...
        def init():
            ...
            line_mercury.set_3d_properties([])
            line_venus.set_data([], [])
            line_venus.set_3d_properties([])
            return line_earth, line_rogue, line_jupiter, line_sun, line_mercury, line_venus
        ...
        def update(frame):
            ...
            line_rogue.set_3d_properties(p_rogue_z[:frame])
            line_mercury.set_data(p_mercury_x[:frame], p_mercury_y[:frame])
            line_mercury.set_3d_properties(p_mercury_z[:frame])
            line_venus.set_data(p_venus_x[:frame], p_venus_y[:frame])
            line_venus.set_3d_properties(p_venus_z[:frame])
            line_sun.set_data([0], [0])
            line_sun.set_3d_properties([0])
            return line_earth, line_rogue, line_sun, line_mercury, line_venus

        total_frames = 15 * 10  # 15 seconds at 10 fps

                # Calculate the step size to ensure the whole video is shown from start to finish
        step_size = len(p_earth_x) // total_frames if len(p_earth_x) // total_frames > 0 else 1
        ...
        if len(p_earth_x) >0:
            ...
            ani.save('Dataset_LC_14.02.08.gif', writer='qt', fps=10)
AxVid()
...
def WriteExcel():
    ...
    excel_file_path = 'Dataset_LC3.xlsx'
...
# Load the degrees data
degrees = pd.read_csv("Degrees.csv", header=None)[0]  # One column

# Get the first angle (in degrees)
first_angle = degrees.iloc[0]

# Find the first index where the angle becomes greater than the first
for idx, angle in enumerate(degrees[1:], start=1):
    if angle > first_angle:
        print(f"First angle: {first_angle}")
        print(f"First greater angle: {angle} at index {idx}")
        break
